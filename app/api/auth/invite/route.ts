  ...
  try {
    ...
    if (profileError || callerProfile?.role !== 'owner') {
      return NextResponse.json(
        {
          error:
            'Permission denied. Only users with the owner role can invite new members.',
        },
        { status: 403 }
      );
    }
    ...
    if (!Array.isArray(segmentIds) || segmentIds.length === 0) {
      return NextResponse.json(
        { error: 'At least one segment must be assigned.' },
        { status: 400 }
      );
    }
    ...
    if (!process.env.SUPABASE_SECRET_KEY) {
      return NextResponse.json(
        {
          error:
            'Missing SUPABASE_SECRET_KEY in .env.local. Please add your Supabase Service Role Key to .env.local to enable sending user invitations.',
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            'Server configuration error: Unable to initialize Supabase Admin client.',
        },
        { status: 500 }
      );
    }
    ...
    const userSegmentRecords = segmentIds.map((segId: string) => ({
    ...
    // 9. Insert optional initial targets configured during invitation
    if (Array.isArray(targets) && targets.length > 0) {
      const targetRows = targets
        .filter(
          (t: any) =>
            (t.segment_id || t.segmentId) &&
            Number(t.target_value ?? t.targetValue) > 0
        )
        .map((t: any) => ({
          segment_id: t.segment_id || t.segmentId,
          user_id: invitedUser.id,
          year: Number(t.year) || new Date().getFullYear(),
          period_type: t.period_type || t.periodType || 'monthly',
          period_value:
      ...
      if (targetRows.length > 0) {
        for (const row of targetRows) {
          const { error: targetInsertErr } = await supabaseAdmin
            .from('targets')
            .insert(row);

          if (targetInsertErr) {
            console.error('[Invite API] Targets insertion error:', targetInsertErr);
          }
        }
      }
    }
    ...
    return NextResponse.json(
      {
        error:
          'An unexpected error occurred while processing the invitation.',
        detail: String(err),
      },
      { status: 500 }
    );
