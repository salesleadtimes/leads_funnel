...
export interface AuthContextType {
  ...
  isOwner: boolean;
  assignedSegments: BusinessSegment[];
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [assignedSegments, setAssigned] = useState<BusinessSegment[]>([]);
      ...
      try {
        ...
        if (!prof) {
          ...
          if (email) {
            ...
            const fallbackRole =
              sessionUser?.user_metadata?.role === 'owner' ? 'owner' : 'member';
        ...
        if (prof) {
          ...
          const activeSegs = (allSegs || []) as BusinessSegment[];
          ...
          if (prof.role === 'owner') {
            ...
            const mapped = ((userSegs || []) as any[])
              .map((r) => r.segments)
              .filter(Boolean) as BusinessSegment[];
            setAssigned(mapped);
            if (mapped.length > 0) {
