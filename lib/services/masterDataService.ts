 ...
 */
export async function fetchCategoriesBySegment(segmentId: string): Promise<any[]> {
  if (!segmentId) return [];
  ...
  return ((data || []) as any[]).map((r) => r.product_categories).filter(Boolean);
}

/**
 * Fetch lead sources available for a given segment.
 */
export async function fetchSourcesBySegment(segmentId: string): Promise<any[]> {
  if (!segmentId) return [];
  const { data, error } = await supabase
    ...
    .eq('segment_id', segmentId);
  if (error) throw error;
  return ((data || []) as any[]).map((r) => r.lead_sources).filter(Boolean);
}

/**
 * Fetch pipeline stages available for a given segment.
 */
export async function fetchStagesBySegment(segmentId: string): Promise<any[]> {
  if (!segmentId) return [];
  const { data, error } = await supabase
    ...
    .order('display_order');
  if (error) throw error;
  return ((data || []) as any[]).map((r) => r.lead_stages).filter(Boolean);
}
...
export async function mapCategoryToSegment(segmentId: string, categoryId: string): Promise<void> {
...
export async function createLeadSource({ code, name }: { code: string; name: string }): Promise<any> {
  const { data, error } = await supabase
    .from('lead_sources')
    .insert({ code, name })
    .select()
...
export async function mapSourceToSegment(segmentId: string, sourceId: string): Promise<void> {
  const { error } = await supabase
    .from('segment_lead_sources')
...
export async function assignSegmentToUser(userId: string, segmentId: string): Promise<void> {
  const { error } = await supabase
    .from('user_segments')
...
export async function removeSegmentFromUser(userId: string, segmentId: string): Promise<void> {
  const { error } = await supabase
    .from('user_segments')
    .delete()
...
export async function fetchUserSegments(userId: string): Promise<any[]> {
  const { data, error } = await supabase
    .from('user_segments')
    .select('segment_id, segments(id, code, name)')
    .eq('user_id', userId);
  if (error) throw error;
  return ((data || []) as any[]).map((r) => r.segments).filter(Boolean);
}
