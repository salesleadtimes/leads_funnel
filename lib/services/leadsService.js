...
export function mapRowToLead(row) {
  ...
  return {
    ...
    category:       row.product_categories || row.segment_product_categories?.product_categories || null,
    source:         row.lead_sources || row.segment_lead_sources?.lead_sources || null,
    stage:          row.lead_stages || row.segment_lead_stages?.lead_stages || null,
    assignedProfile: row.assigned_profile || row.profiles || row.profiles_leads_assigned_to_fkey || null,
...
export async function getLeads({ segmentId = null, includeGemBids = true } = {}) {
  ...
  if (error) {
    // Graceful fallback query if composite relation syntax differs
    ...
    return (res.data || []).map(mapRowToLead);
  }
